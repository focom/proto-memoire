...
def createQuery(name):
    ...
    //Level 2\
    (Ceasy:Chapitre {name:'Contenus d\'apprentissage',grade:0,id_node:6,history:0}), (Chard:Chapitre {name:'Contenus d\'appronfondissement',grade:0,id_node:7,history:0}),\
    (Tphrase:Chapitre {name:'Type de phrase',grade:0,id_node:8,history:0}), (conjugaison:Chapitre {name:'Contenus pour l\'accord du verbe',grade:0,id_node:9,history:0}),\
...
\
    ...
    (:Exercice {id_node:17,instruction:'Trouve le complement de la phrase', question:'La petite s\'excusera à la prochaine visite', answer:['La petite','à la prochaine visite'], correct:'à la prochaine visite'})-[:link]->(Ceasy),\
    ...
    (:Exercice {id_node:19,instruction:'Trouve le complement de la phrase', question:'J\'aime la pizza avec du poivron', answer:['la pizza','avec du poivron'], correct:'avec du poivron'})-[:link]->(Ceasy),\
    ...
    (:Exercice {id_node:23,instruction:'Trouve le verbe', question:'Morgane trouve un mouton rouge dans le champ', answer:['meme si','j\'aime'], correct:'j\'aime'})-[:link]->(Ceasy),\
    (:Exercice {id_node:24,instruction:'Trouve le verbe', question:'La vache broute l\'herbe', answer:['broute','l\'herbe'], correct:'broute'})-[:link]->(Ceasy),\
\
    // Exercice 2\
    ...
    (:Exercice {id_node:33,instruction:'Trouve le complement de la phrase', question:'Le soir, à la maison, mes parents m\'aident', answer:['à la maison','m\'aident'], correct:'à la maison'})-[:link]->(Chard),\
...
\
    ...
    (:Exercice {id_node:40,instruction:'Identifie le type de phrase', question:'Comme il avait l\'air triste !', answer:['declaratif','exclamatif'], correct:'declaratif'})-[:link]->(Tphrase),\
...
\
    ...
    (:Exercice {id_node:56,instruction:'choisis la bonne orthographe', question:'C\'est moi qui (etre) votre collaborateur', answer:['est','suis'], correct:'suis'})-[:link]->(conjugaison),\
...
\
    ...
    (:Exercice {id_node:69,instruction:'choisis la bonne orthographe', question:'La semaine derniere _ nous avons mangé, c\'etait un bon restaurant.', answer:['où','ou'], correct:'où'})-[:link]->(omonyme),\
    (:Exercice {id_node:70,instruction:'choisis la bonne orthographe', question:'_ DVD sont les miens', answer:['ces','c\'est'], correct:'ces'})-[:link]->(omonyme),\
    (:Exercice {id_node:71,instruction:'choisis la bonne orthographe', question:'_ voitures sont belles', answer:['ces','c\'est'], correct:'c\'est'})-[:link]->(omonyme),\
    (:Exercice {id_node:72,instruction:'choisis la bonne orthographe', question:'_ tout se que j\'aime', answer:['ces','c\'est'], correct:'c\'est'})-[:link]->(omonyme),\
...
\
    ...
    (:Exercice {id_node:77,instruction:'choisis la bonne orthographe', question:'Ce tableau est un (chef d\'oeuvre/chef-d\'oeuvre)', answer:['chef d\'oeuvre','chef-d\'oeuvre'], correct:'chef-d\'oeuvre'})-[:link]->(autre),\
    (:Exercice {id_node:78,instruction:'choisis la bonne orthographe', question:'J\'ai recu un (coup-de-pied/coup de pied)', answer:['coup-de-pied','coup de pied'], correct:'coup de pied'})-[:link]->(autre),\
    ...
    (:Exercice {id_node:81,instruction:'choisis la bonne orthographe', question:'Venez me (s\'il-vous-plait/s\'il vous plait)', answer:['s\'il-vous-plait','s\'il vous plait'], correct:'s\'il vous plait'})-[:link]->(autre),\
    (:Exercice {id_node:82,instruction:'choisis la bonne orthographe', question:'Je suis (tout-à-fait/tout à fait) d\'accord', answer:['tout-à-fait','tout à fait'], correct:'tout à fait'})-[:link]->(autre),\
    (:Exercice {id_node:83,instruction:'choisis la bonne orthographe', question:'Un manteau pourpre (c\'est-a-dire/c\'est a dire) rouge foncé', answer:['c\'est-a-dire','c\'est a dire'], correct:'c\'est-a-dire'})-[:link]->(autre),\
